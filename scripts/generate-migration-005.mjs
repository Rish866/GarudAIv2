...
function generateBlockA_check4(L) {
  ...
  L.push("      WHEN con.contype != 'f' THEN 'not FK (type=' || con.contype::text || ')'");
  L.push("      WHEN NOT con.convalidated THEN 'NOT VALIDATED'");
  L.push("      WHEN con.confmatchtype != 's' THEN 'MATCH type=' || con.confmatchtype::text");
  L.push("      WHEN con.confupdtype != 'a' THEN 'ON UPDATE=' || con.confupdtype::text");
  L.push("      WHEN con.confdeltype != 'a' THEN 'ON DELETE=' || con.confdeltype::text");
...
function generateBlockC() {
  ...
  L.push("  CASE WHEN count(*) = 0 THEN 'PASS'");
  L.push("    ELSE 'FAIL: ' || string_agg(i.conname || '(v=' || i.convalidated::text || ',m=' || i.confmatchtype::text || ',u=' || i.confupdtype::text || ',d=' || i.confdeltype::text || ')', ', ')");
  L.push('  END');
...
function generateBlockD() {
  ...
  L.push('-- and automatically rolls back via exception-backed subtransaction.');
  L.push('-- Run ONLY after Block C passes and operator confirms.');
  L.push('-- Design: outer DO block captures baselines, inner BEGIN/EXCEPTION block');
  L.push('-- performs all DML then intentionally raises to trigger rollback.');
  L.push('-- Post-block SELECT compares current counts to baselines.');
  L.push('-- No temp tables needed — baselines stored in PL/pgSQL variables.');
  L.push('');
  L.push('DO $wrapper$');
  L.push('DECLARE');
  L.push('  -- Baseline counts (captured before any test DML)');
  L.push('  b_organizations BIGINT; b_customers BIGINT; b_drivers BIGINT;');
  L.push('  b_vehicles BIGINT; b_trips BIGINT; b_enquiries BIGINT;');
  L.push('  b_quotations BIGINT; b_invoices BIGINT; b_vendors BIGINT;');
  L.push('  b_branches BIGINT; b_expenses BIGINT;');
  L.push('  -- Post-rollback counts');
  L.push('  a_organizations BIGINT; a_customers BIGINT; a_drivers BIGINT;');
  L.push('  a_vehicles BIGINT; a_trips BIGINT; a_enquiries BIGINT;');
  L.push('  a_quotations BIGINT; a_invoices BIGINT; a_vendors BIGINT;');
  L.push('  a_branches BIGINT; a_expenses BIGINT;');
  L.push('  -- Test variables');
  ...
  L.push('  mismatches TEXT[];');
  L.push('BEGIN');
  L.push('  -- Record baselines');
  L.push('  SELECT count(*) INTO b_organizations FROM public.organizations;');
  L.push('  SELECT count(*) INTO b_customers FROM public.customers;');
  L.push('  SELECT count(*) INTO b_drivers FROM public.drivers;');
  L.push('  SELECT count(*) INTO b_vehicles FROM public.vehicles;');
  L.push('  SELECT count(*) INTO b_trips FROM public.trips;');
  L.push('  SELECT count(*) INTO b_enquiries FROM public.enquiries;');
  L.push('  SELECT count(*) INTO b_quotations FROM public.quotations;');
  L.push('  SELECT count(*) INTO b_invoices FROM public.invoices;');
  L.push('  SELECT count(*) INTO b_vendors FROM public.vendors;');
  L.push('  SELECT count(*) INTO b_branches FROM public.branches;');
  L.push('  SELECT count(*) INTO b_expenses FROM public.expenses;');
  L.push('');
  L.push('  -- Inner subtransaction: all test DML rolls back on the intentional RAISE');
  L.push('  BEGIN');
  L.push('    -- Setup');
  ...
  L.push('    EXCEPTION WHEN foreign_key_violation THEN violation_caught := TRUE;');
  L.push('    END;');
  L.push("    IF NOT violation_caught THEN RAISE EXCEPTION 'TEST 2 FAIL: cross-org not rejected'; END IF;");
  ...
  L.push('    EXCEPTION WHEN foreign_key_violation THEN violation_caught := TRUE;');
  L.push('    END;');
  L.push("    IF NOT violation_caught THEN RAISE EXCEPTION 'TEST 3 FAIL: nonexistent not rejected'; END IF;");
  ...
  L.push('    EXCEPTION WHEN foreign_key_violation THEN violation_caught := TRUE;');
  L.push('    END;');
  L.push("    IF NOT violation_caught THEN RAISE EXCEPTION 'TEST 6 FAIL: cross-org circular not rejected'; END IF;");
  ...
  L.push('');
  L.push('    -- Intentionally raise to roll back all test DML');
  L.push("    RAISE EXCEPTION '_m005_deliberate_rollback';");
  L.push("  EXCEPTION WHEN OTHERS THEN");
  L.push("    IF SQLERRM != '_m005_deliberate_rollback' THEN");
  L.push('      RAISE;  -- Re-raise unexpected errors');
  L.push('    END IF;');
  L.push('    -- Deliberate rollback: subtransaction DML is undone');
  L.push('  END;');
  L.push('');
  L.push('  -- Verify row counts match baselines (subtransaction rolled back)');
  L.push('  SELECT count(*) INTO a_organizations FROM public.organizations;');
  L.push('  SELECT count(*) INTO a_customers FROM public.customers;');
  L.push('  SELECT count(*) INTO a_drivers FROM public.drivers;');
  L.push('  SELECT count(*) INTO a_vehicles FROM public.vehicles;');
  L.push('  SELECT count(*) INTO a_trips FROM public.trips;');
  L.push('  SELECT count(*) INTO a_enquiries FROM public.enquiries;');
  L.push('  SELECT count(*) INTO a_quotations FROM public.quotations;');
  L.push('  SELECT count(*) INTO a_invoices FROM public.invoices;');
  L.push('  SELECT count(*) INTO a_vendors FROM public.vendors;');
  L.push('  SELECT count(*) INTO a_branches FROM public.branches;');
  L.push('  SELECT count(*) INTO a_expenses FROM public.expenses;');
  L.push('');
  L.push('  mismatches := ARRAY[]::TEXT[];');
  L.push("  IF a_organizations != b_organizations THEN mismatches := mismatches || ('organizations: ' || b_organizations || '->' || a_organizations); END IF;");
  L.push("  IF a_customers != b_customers THEN mismatches := mismatches || ('customers: ' || b_customers || '->' || a_customers); END IF;");
  L.push("  IF a_drivers != b_drivers THEN mismatches := mismatches || ('drivers: ' || b_drivers || '->' || a_drivers); END IF;");
  L.push("  IF a_vehicles != b_vehicles THEN mismatches := mismatches || ('vehicles: ' || b_vehicles || '->' || a_vehicles); END IF;");
  L.push("  IF a_trips != b_trips THEN mismatches := mismatches || ('trips: ' || b_trips || '->' || a_trips); END IF;");
  L.push("  IF a_enquiries != b_enquiries THEN mismatches := mismatches || ('enquiries: ' || b_enquiries || '->' || a_enquiries); END IF;");
  L.push("  IF a_quotations != b_quotations THEN mismatches := mismatches || ('quotations: ' || b_quotations || '->' || a_quotations); END IF;");
  L.push("  IF a_invoices != b_invoices THEN mismatches := mismatches || ('invoices: ' || b_invoices || '->' || a_invoices); END IF;");
  L.push("  IF a_vendors != b_vendors THEN mismatches := mismatches || ('vendors: ' || b_vendors || '->' || a_vendors); END IF;");
  L.push("  IF a_branches != b_branches THEN mismatches := mismatches || ('branches: ' || b_branches || '->' || a_branches); END IF;");
  L.push("  IF a_expenses != b_expenses THEN mismatches := mismatches || ('expenses: ' || b_expenses || '->' || a_expenses); END IF;");
  L.push('');
  L.push('  IF array_length(mismatches, 1) > 0 THEN');
  L.push("    RAISE EXCEPTION 'D_ROLLBACK FAIL: row counts changed: %', array_to_string(mismatches, ', ');");
  L.push('  END IF;');
  L.push("  RAISE NOTICE 'D_ROLLBACK PASS: all 11 table counts match baselines';");
  L.push('END $wrapper$;');
  L.push('');
  L.push('-- Final confirmation (always reached if DO block succeeds)');
  L.push("SELECT 'D_ROLLBACK' AS check_id, 'row_counts_match_baselines' AS check_name, 'PASS' AS result;");
  L.push('');
  return L.join('\n');
}
