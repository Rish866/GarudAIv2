-- ============================================================
-- GARUD AI ERP — Migration 013: Billing Transaction Integrity
--
...
--
-- DEPENDS ON: organizations, customers, invoices, payments tables
-- ============================================================

-- ============================================================
-- RPC 1: record_payment
-- Atomically: inserts payment, updates invoice, updates customer outstanding
-- Guards: overpayment, cross-org, paid/cancelled invoice
-- ============================================================
...
BEGIN
  -- Verify caller is org member with billing role
  IF NOT public.has_organization_role(p_organization_id, ARRAY[
    'organization_owner', 'admin', 'accountant', 'operations_manager'
  ]) THEN
    RETURN json_build_object('success', false, 'error', 'Not authorized to record payments');
  END IF;
  ...
  IF NOT FOUND THEN
    RETURN json_build_object('success', false, 'error', 'Customer not found in this organization');
  END IF;
  ...
  IF p_invoice_id IS NOT NULL THEN
    ...
    IF v_invoice.customer_id != p_customer_id THEN
      RETURN json_build_object('success', false, 'error', 'Invoice does not belong to this customer');
    END IF;
    ...
    IF v_total_payment > v_invoice.balance_amount THEN
      RETURN json_build_object('success', false, 'error', 'Payment (' || v_total_payment || ') exceeds invoice balance (' || v_invoice.balance_amount || ')');
    END IF;
  END IF;
  ...
  -- 3. Update customer outstanding (never go negative)
  ...
  RETURN json_build_object(
    ...
    'invoice_status', CASE WHEN p_invoice_id IS NOT NULL THEN v_new_status ELSE NULL END
  );
END;
...
-- Guards: duplicate number, cross-org customer
-- ============================================================
...
BEGIN
  -- Verify caller has billing role
  IF NOT public.has_organization_role(p_organization_id, ARRAY[
    'organization_owner', 'admin', 'accountant', 'operations_manager'
  ]) THEN
    RETURN json_build_object('success', false, 'error', 'Not authorized to create invoices');
  END IF;
  ...
  IF EXISTS (
    ...
    RETURN json_build_object('success', false, 'error', 'Invoice number ' || p_invoice_number || ' already exists');
  END IF;
  ...
  RETURN json_build_object(
    ...
    'customer_outstanding', COALESCE(v_customer.outstanding, 0) + v_total_amount
  );
END;
$$ LANGUAGE plpgsql SECURITY DEFINER SET search_path = public;

-- ============================================================
-- DONE — Migration 013: Billing Integrity
-- Transaction-safe billing: record_payment + create_invoice_with_outstanding
-- Prevents: overpayment, negative outstanding, cross-org, duplicates
-- ============================================================
